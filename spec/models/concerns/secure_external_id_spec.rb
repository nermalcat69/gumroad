...
RSpec.describe SecureExternalId, type: :model do
  ...
  describe '#secure_external_id' do
    ...
    it 'generates a token with a specific purpose' do
      token = product.secure_external_id(purpose: 'test_purpose')
      found_product = SecurableProduct.find_by_secure_external_id(token, purpose: 'test_purpose')
      expect(found_product).to eq(product)
    end
  end
  ...
  describe '.find_by_secure_external_id' do
    ...
    it 'returns nil for a token used with the wrong purpose' do
      token = product.secure_external_id(purpose: 'correct_purpose')
      expect(SecurableProduct.find_by_secure_external_id(token, purpose: 'wrong_purpose')).to be_nil
    end
  ...
  describe 'key rotation' do
    ...
    it 'can decrypt a token created with an old key (v1)' do
      ...
      inner_payload = { model: 'SecurableProduct', id: product.id, pur: 'default' }
